...
def create_dataset():
    ...
    x[:],y[:] = zip(*c) #extract from shuffled data from zip in x and y list 
    ...
    y = ['starttoken '+l+' endtoken' for l in y]
    y = [l.replace('%','percent') for l in y]
    tokenizer = Tokenizer() #creates tokenizer object
    tokenizer.fit_on_texts(y)  #maps each word to an index and replaces it with that index (inplace operation)
    save_file(tokenizer,TOKENIZER_PATH) #saved out word to index mapping dictionary
    x_train,y_train,x_val,y_val,x_test,y_test = split_dataset(x,y) #spliting dataset
    save_file((x_test,y_test),RAW_TEST_DATASET_PATH) #saves test dataset for testing later
...
def create_model(x1,x2,y):
    ...
    att = Dense(x1.shape[1],activation='softmax')(input1)
    mul = Multiply()([input1,att])
    input2 = Input(shape=(x2.shape[1],))
    emb = Embedding(y.shape[1],128,mask_zero=True)(input2)
    lstm1 = Bidirectional(LSTM(256,return_sequences=True))(emb)
    lstm2 = Bidirectional(LSTM(256,return_sequences=True))(lstm1)
    lstm3 = Bidirectional(LSTM(256))(lstm2)
    con = concatenate([lstm3,mul])
