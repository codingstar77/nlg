...
from keras.layers import Input,LSTM,Dense,Embedding,Dropout,Bidirectional,Multiply
...
MODEL_PATH = './model.h5'
RAW_TEST_DATASET_PATH = './rawtest.pkl'
PROCESSED_TEST_DATASET_PATH = './test.pkl'
MAX_COUNT = 29000
MAX_LEN_FEATURES = 78
MAX_LEN_DESC = 78
BATCH_SIZE = 2048
...
def create_model(x1,x2,y):
    ...
    con = concatenate([lstm2,merge1])
    #dec1 = Dense(256,activation='relu')(con)
    dec2 = Dense(y.shape[1],activation='softmax')(con)
...
model.fit([x1_train,x2_train],y_train, epochs=30, batch_size=BATCH_SIZE, callbacks=[checkpoint], verbose=1,validation_data=([x1_val,x2_val],y_val))
